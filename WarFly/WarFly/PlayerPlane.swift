...
class PlayerPlane: SKSpriteNode {
    ...
    fileprivate func turnPlane(direction: TurnDirection) {
        ...
        let forwardActoin = SKAction.animate(with: array, timePerFrame: 0.05, resize: true, restore: false)
        let backwardAction = SKAction.animate(with: array.reversed(), timePerFrame: 0.05, resize: true, restore: false)
        let sequenceAction = SKAction.sequence([forwardActoin, backwardAction])
